...
public class Main {
    public static void main(String[] args) throws IOException {
        char[] ones = new char[3];
        char[] zeros = new char[3];
        char[] initials = { 'A', 'B', 'C' };
        byte n_ones=0, n_zeros=0;
        String play;
        ...
        while (scanner.hasNext()) {
            for (byte i=0; i<3; i++) {
                play = scanner.next();
                if (play.charAt(0) == '1') ones[n_ones++] = initials[i];
                if (play.charAt(0) == '0') zeros[n_zeros++] = initials[i];
            }

            System.out.println(n_ones == 1 ?
                    ones[0] :
                    n_zeros == 1 ?
                            zeros[0] :
                            '*');
